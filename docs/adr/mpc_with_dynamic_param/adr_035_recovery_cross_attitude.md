...
## ステータス
**操舵の符号の決め方は ADR-044 で変更**。角度差に比例させると、既に目標を越えて
刺さっているとき (e_y > 0 で e_psi < -40度) に符号が反転し「左で停止したのに
後退で右に切る」契約違反が起きていた。向きは停止側で固定し、大きさだけを
角度差に比例させる形にした。交差姿勢という目標そのものは維持。
