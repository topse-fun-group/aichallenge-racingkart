...
## ステータス
承認済み (Accepted) — ただし**停止・徐行している先行車は ADR-039 で例外**とした。
本番ログで、内側に 4.47m 空いた停止車の後ろで veto が効き続け、
stuck -> recovery を 10 回反復して 41.8s (走行の 8%) を失っていたため。
...
2. **`forward_vehicle_gap` が常に 0.0 で `is_ttc_close` が恒真**（ADR-027 から継続）。
   本番では突入の 37〜41% が自車より速い相手（相対速度が負）で、成功率 17〜18%。
   `safe_width` に相対速度ゲートが無いことの実害が本番で確認された（ADR-039）。
